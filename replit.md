...
- `GET /api/actions/:date` — Get consolidated action items for the day (no LLM cost — extracted from existing analysisJson). 7 types: `follow_up_pendente`, `risco_perda_lead`, `desvio_procedimento`, `qualidade_critica`, `oportunidade_cross_sell`, `cotacao_sem_seguimento`, `lead_quente_sem_fecho`
